...
// src/next.ts
function withPageXerox(options) {
  return async function run(dir) {
    const { existsSync } = await import("fs");
    const { resolve } = await import("path");
    const outputDir = dir ?? (existsSync(resolve("out")) ? resolve("out") : null) ?? resolve(".next/server/pages");
